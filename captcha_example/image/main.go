...
func trainloadsave() {
	//err:=jxocr.ImageColorInfo(in,out string) ==> threshhole = 34000
	//err=jxocr.DownCaptcha(dir string, n int) ==> get captcha..
	...
	// 写入 样本....
	for i := 0; i < 197; i++ {
		img, err := jxocr.ReadImg(`/Users/kqwang/development/gowork/src/github.com/892294101/jxocr/captcha_example/验证码图片/` + strconv.Itoa(i) + ".jpg")
		if err != nil {
			fmt.Println(err)
			continue
			//log.Fatal(err)
	...
	for char, binimgs := range trainModule {
		...
		for f, binimg := range binimgs {
			fmt.Println("file: ", f, binimg)
		}
	}
...
func recognize() {
	...
	stdModule, err := c.LoadStdModule(`/Users/kqwang/development/gowork/src/github.com/892294101/jxocr/captcha_example/Std.dat`)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	/*c := jxocr.NewCaptcha(Threshhole, N)
	train, err := c.LoadTrainModule(`/Users/kqwang/development/gowork/src/github.com/892294101/jxocr/captcha_example/Training.dat`)
	if err != nil {
		log.Fatal(err)
	}
